 ...
 */
#ifndef lint
static char rcsid[] = "@(#)$RCSfile: lzw.c,v $ $Revision: 1.1.20.1 $ (DEC) $Date: 2003/01/06 22:45:36 $";
#endif
...
typedef struct {
    int  wK;     /* w is in upper 16 bits and K is in lower 16 bits */
    int  wKcode; /* code for wK                                      */
} table_entry_t;
...
typedef struct {
     ...
     */
    int new_uncompress; /* flag indicates whether a new set of buffers      */
                    /*    are being uncompressed                            */
    int reset;      /* is the new_uncompress due to a RESET?                */
    int old_code;   /* previous code read from input                        */
    int next_code;  /* next available code                                  */
    int fin_char;   /* the last character decoded from a code (however it's */
                    /* actually the first character of the final string)    */
...
#define STK_POP()\
   ((lzw->stk.top > 0) ? \
    ...
    (fprintf( stderr, catgets(comcatd, S_LZW1, LZW2, "\n**** compression stack underflow ****\n") ), \
     kill( getpid(), SIGQUIT )))
        ...
        if (lzw->new_uncompress) {
            ...
            if (!lzw->reset) {
                ...
                if (new_code != COMPRESSED_TAG) {
                    fprintf( stderr, catgets(comcatd, S_LZW1, LZW3, "Data is not in compressed format.\n") );
                    return ERROR;
                }
            }
