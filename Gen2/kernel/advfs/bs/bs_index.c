...
#include <sys/stat.h>
#include <sys/vfs.h>                  /* struct vfsops for fs/vfs_ifaces.h */
...
idx_setup_for_truncation_int(
    ...
    bs_meta_page_t      page_to_search,
    uint32_t            level,
    uint64_t            *undo_space_start,
    int                 operation,
    ftxHT               ftxH,
    int32_t             *partial_truncate
    );
...
struct
{
    uint64_t insert_space_failed;
    uint64_t get_space_failed;
    uint64_t prune_btree_incomplete;
    uint64_t prune_btree_failed;
    uint64_t get_free_pgs_failed;
    uint64_t remove_file_failed;
    uint64_t setup_for_truncation_failed;
    uint64_t remove_page_of_storage_failed;
    uint64_t redistribute_right;
    uint64_t prune_root_1_failed;
    uint64_t prune_root_2_failed;
}idxStats;
    ...
    )
{
    ...
    sts = FTX_START_N(FTA_IDX_UNDO_V1,
                      &create_ftxH, parentFtxH, parentFtxH.dmnP);
    if (sts != EOK)
    {
    ...
    /*
     * Make the page size for the index match that of the directory.
     */
    bfParams.bfpPageSize =      ADVFS_METADATA_PGSZ_IN_FOBS;
    bfParams.cl.reqServices =   nilServiceClass;

    sts = rbf_create(&indexTag,dir_bfap->bfSetp,&bfParams,create_ftxH, 
            CRT_CHK_MIGSTG_LOCK, BFD_METADATA);

    if (sts != EOK)
    {
        ADVFS_CRASH_RECOVERY_TEST(parentFtxH.dmnP, AdvfsCrash_390);
        ftx_fail(create_ftxH);
        ADVFS_CRASH_RECOVERY_TEST(parentFtxH.dmnP, AdvfsCrash_391);
        return(sts);
    }
    ...
    if (sts != EOK)
    {
        ADVFS_CRASH_RECOVERY_TEST(parentFtxH.dmnP, AdvfsCrash_392);
        ftx_fail(create_ftxH);
        ADVFS_CRASH_RECOVERY_TEST(parentFtxH.dmnP, AdvfsCrash_393);
        return(sts);
    }
    ...
    sts = chk_blk_quota(&dir_bfap->bfVnode,
                        2 * ADVFS_METADATA_PGSZ_IN_FOBS,
                        TNC_CRED(),
                        0,
                        create_ftxH);
    ...
    /*
     * Add 2 pages of storage (page 0 and 1). 
     */
    sts = rbf_add_stg( idx_bfap, 0, 2 * idx_bfap->bfPageSz, create_ftxH, STG_NO_FLAGS );

    if (sts != EOK) goto idx_create_error;

    /* Get memory for the index parameters that hang off the
     * directory's access structure. The memory for this will be
     * released in advfs_lookup_valid_bfap when the access structure 
     * is recycled
     */

    dir_idx_params = (bsDirIdxRecT *) ms_malloc_no_bzero(sizeof(bsDirIdxRecT));
     ...
     * advfs_lookup_valid_bfap when the access structure is recycled.  
     */

    idx_params = (bsIdxRecT *) ms_malloc_no_bzero(sizeof(bsIdxRecT));
    ...
    idx_params->bmt.seqDirTag = dir_bfap->tag;
    ...
    ADVFS_CRASH_RECOVERY_TEST(parentFtxH.dmnP, AdvfsCrash_181);
    ftx_done_u (create_ftxH, FTA_IDX_UNDO_V1,
                  sizeof(idxUndoRecordT), &undorec);
    ADVFS_CRASH_RECOVERY_TEST(parentFtxH.dmnP, AdvfsCrash_182);

    /* This deserves some explanation. The bmtr_put_rec routine
     * does not supply an undo for the creation of a record. Since
     * it starts a subftx, the record could get out to disk at
     ...
     * window would exist between putting the record out and doing the done.
    ...
    if (sts != EOK)
    {
        ...
        bs_close(idx_bfap, MSFS_CLOSE_NONE);
...
idx_create_error:
    ...
    ADVFS_CRASH_RECOVERY_TEST(parentFtxH.dmnP, AdvfsCrash_394);
    ftx_fail(create_ftxH);
    ADVFS_CRASH_RECOVERY_TEST(parentFtxH.dmnP, AdvfsCrash_395);

    return (sts);
}

/*********************************************************************
 * This will be called from advfs_rmdir when the directory is removed.
 ...
 * enough level such that the snapshot processing will be done correctly.
 *
 ...
 *                access structure is first initialized. It is also
 ...
 *                and its mcell does not need to be checked. This saves
 ...
 *
 * If the access structure has been recycled, then this function will get memory
 * and re-read from disk the info for accessing the index file. Next it
    ...
    )
{
    ...
    if ((bsDirIdxRecT*) dir_bfap->idx_params == NULL)
    {
        ...
        if (dir_bfap->bfaFlags & BFA_VIRGIN_SNAP) {
            /* The parent directory is a virgin snapshot.  The parent may be
             * migrated out from under us in an rmvol so start a transaction
             * to syncrhonize if one isn't already started.
             */
            if (!FTX_EQ(ftxH, FtxNilFtxH)) {
                sts = FTX_START_N( FTA_NULL,&sync_ftx, ftxH,
                        dir_bfap->dmnP);
                if (sts != EOK) {
                    /* There isn't much we can do in this case.  Ideally,
                     * we'd just mark the child out of sync and continue,
                     * but here, we failed to start a transaction and the
                     * domain is probably in bad shape.
                     */
                    domain_panic( dir_bfap->dmnP, 
                            "idx_open_index_file: failed to start a transaction");
                    return sts;
                }
                finish_ftx = TRUE;
            }
        ...
        if (finish_ftx) {
            ADVFS_CRASH_RECOVERY_TEST(dir_bfap->dmnP, AdvfsCrash_183);
            ftx_done_n( sync_ftx, FTA_NULL );
            ADVFS_CRASH_RECOVERY_TEST(dir_bfap->dmnP, AdvfsCrash_184);
        }

        MS_SMP_ASSERT(index_tag.tag_num >  0);

        /*
         * Get memory for the index parameters that hang off the
         * directory's access structure. The memory for this will be
         * released in advfs_lookup_valid_bfap when the access structure is
         * recycled
         */
 ...
 * recycled (in advfs_lookup_valid_bfap) or released.
 *
 * Snapshots processing will work without needing to associate the original
 * dir and index together. Only the snapshot dir and index will need to be
 * associated.
 *
 *********************************************************************/
void
    ...
    )
{
    ...
    case IDX_REMOVING_DIR :
        ...
        dir_bfap->idxQuotaBlks = 
            QUOTABLKS((uint64_t)idx_bfap->xtnts.bimxAllocFobCnt *
                      ADVFS_FOB_SZ);
    ...
    sts = bs_close(idx_bfap, MSFS_CLOSE_NONE);
...
idx_bsearch_int(
    ...
    uint64_t search_key,    /* value to search for */
...
idx_index_get_free_pgs_int(
    ...
    bs_meta_page_t  count,      /* Number of pages required */
    idxPinnedPgsT   *pptr,      /* out - pages we obtained */
    ftxHT           ftxH        /* Parent transaction */
    )
{
    uint32_t i;
    bs_meta_page_t holePg, newPg;
    bs_meta_page_t holePgCnt;
    statusT sts;
     ...
     * all callers of this function currently hold the migStgLk 
     * exclusively. This is enough to prevent the xtntmap from changing
     * out from under us.
     *
     * imm_get_first_hole works in fobs.  We will get fobs back, then
     * convert to index pages.
     */
 
    imm_get_first_hole( &idx_bfap->xtnts, &holePg, &holePgCnt );
    holePg = holePg / idx_bfap->bfPageSz;
    holePgCnt = holePgCnt / idx_bfap->bfPageSz;

    if (holePgCnt < count)
    {
        /* Extend the file, no holes were found.*/
        newPg = idx_bfap->bfaNextFob / idx_bfap->bfPageSz;
    }
    else newPg = holePg;

    sts = chk_blk_quota(
                        &((bsIdxRecT *)idx_bfap->idx_params)->dir_bfap->bfVnode,
                        count * ADVFS_METADATA_PGSZ_IN_FOBS,
                        TNC_CRED(),
                        0,
                        ftxH);
    if (sts != EOK) {
    ...
    sts = rbf_add_stg( idx_bfap, 
                       newPg * idx_bfap->bfPageSz, /* start fob */
                       count * ADVFS_METADATA_PGSZ_IN_FOBS, /* Fob count */
                       ftxH, 
                       STG_MIG_STG_HELD );
...
static void
idx_split_node_int(
    uint64_t        search_key,    /* value to be inserted */
    uint64_t        value,        /* value to be inserted */
    uint64_t        index,         /* where to put it before split */
    ...
    )
{
    ...
    if (index <= idx_middle)
    {
        ...
        copy_node(&(src_node_ptr->data[idx_middle]),
                  &(dst_node_ptr->data[0]),
                  idx_dst);
        ADVFS_CRASH_RECOVERY_TEST(dst_pgref.dmnP, AdvfsCrash_185);
        ...
        copy_node(&(src_node_ptr->data[index]),
                  &(src_node_ptr->data[index+1]),
                  (idx_src - index));
        ADVFS_CRASH_RECOVERY_TEST(src_pgref.dmnP, AdvfsCrash_186);
    ...
    else
    {
        ...
        copy_node(&(src_node_ptr->data[idx_src]),
                  &(dst_node_ptr->data[0]),
                  (index - idx_src));
        ADVFS_CRASH_RECOVERY_TEST(src_pgref.dmnP, AdvfsCrash_187);
        ...
        {
            copy_node(&(src_node_ptr->data[index]),
                      &(dst_node_ptr->data[index - idx_src + 1]),
                      (IDX_MAX_ELEMENTS - index));
        }
    ...
    )
{
    ...
    search_key = idx_hash_filename_int(name);
    ...
    if ((level > 0) && (ndp->ni_hint != ADVFS_IDX_HINT_INVAL) )
    {
        root_page = (bs_meta_page_t)ndp->ni_hint;
        level = 0;
        ndp->ni_hint = ADVFS_IDX_HINT_INVAL;
    }
    else
    {
    ...
    location = IDX_GET_ADDRESS(page,count, idx_bfap);
    ...
    if (sts != EOK)
    {
        ADVFS_CRASH_RECOVERY_TEST(parentFtx.dmnP, AdvfsCrash_396);
        ftx_fail(ftxH);
        ADVFS_CRASH_RECOVERY_TEST(parentFtx.dmnP, AdvfsCrash_397);
        return(sts);
    }
    ...
    if (insert)
    {
        ...
        RBF_PIN_FIELD(pinned_pages[level].pgref,
                      new_root->page_right);
        ADVFS_CRASH_RECOVERY_TEST(parentFtx.dmnP, AdvfsCrash_188);
        ...
        if (sts != EOK)
        {
            ...
            ADVFS_CRASH_RECOVERY_TEST(ftxH.dmnP, AdvfsCrash_189);
            ftx_done_n(ftxH,FTA_IDX_UNDO_V1);
            ADVFS_CRASH_RECOVERY_TEST(ftxH.dmnP, AdvfsCrash_190);
            return(sts);
        }
    }
    ...
    undorec.bfSetId = ((bfSetT *)idx_bfap->bfSetp)->bfSetId;
    undorec.dir_tag = dir_bfap->tag;
    undorec.nodeEntry.search_key = idx_hash_filename_int(name);
    undorec.nodeEntry.loc.dir_offset =
        IDX_GET_ADDRESS(page,count, idx_bfap);
    ...
    ADVFS_CRASH_RECOVERY_TEST(ftxH.dmnP, AdvfsCrash_191);
    ftx_special_done_mode ( ftxH, FTXDONE_SKIP_SUBFTX_UNDO);
    ADVFS_CRASH_RECOVERY_TEST(ftxH.dmnP, AdvfsCrash_192);
    ftx_done_u( ftxH,
                FTA_IDX_UNDO_V1,
                sizeof(idxUndoRecordT),
                (void *) &undorec);
    ADVFS_CRASH_RECOVERY_TEST(ftxH.dmnP, AdvfsCrash_193);
...
static statusT
idx_insert_filename_int(
    bs_meta_page_t      node_page,   /* in - starting node page*/
    uint64_t            level,       /* caller's level in the tree */
    uint64_t            *search_key, /* in - key to insert
                                        out - key to insert*/
    off_t               *offset,     /* in - location to store in directory
                                        out - page location to store in node*/
    bfAccessT           *idx_bfap,   /* b-tree access structure */
    uint32_t            *insert,     /* boolean indicating if an insertion
    ...
    )
{
    ...
    if (*insert)
    {
	    ...
	    ADVFS_CRASH_RECOVERY_TEST(ftxH.dmnP, AdvfsCrash_4);
        ...
        else
        {
            ...
            if (index < node_ptr->total_elements-1)
                copy_node(&node_ptr->data[index],
                          &node_ptr->data[index+1],
                          (node_ptr->total_elements-index-1));
            ADVFS_CRASH_RECOVERY_TEST(ftxH.dmnP, AdvfsCrash_194);
...
idx_remove_filename(
    ...
    bs_meta_page_t    page,     /* Directory page where filename lives */
    off_t             count,    /* Offset on page where filename lives */
    ftxHT             parentFtx /* Parent ftx */
    )
{
    uint64_t search_key;
    uint32_t level;
    bfAccessT *idx_bfap;
    uint64_t location;          /* Both a page value and a byte value */
    bs_meta_page_t root_page;
    ftxHT  ftxH;
    idxUndoRecordT undorec;
    statusT sts;
    ...
    sts = FTX_START_N( FTA_NULL, &ftxH, parentFtx, dir_bfap->dmnP);
    if (sts != EOK)
    {
    ...
    do
    {
        ...
        if (sts != EOK)
        {
            ADVFS_CRASH_RECOVERY_TEST(parentFtx.dmnP, AdvfsCrash_398);
            ftx_fail(ftxH);
            ADVFS_CRASH_RECOVERY_TEST(parentFtx.dmnP, AdvfsCrash_399);
            return(sts);
        }
        ...
        if(!found)
        {
            if(!using_hint)
            {
                ...
                ADVFS_CRASH_RECOVERY_TEST(parentFtx.dmnP, AdvfsCrash_400);
                ftx_fail(ftxH);
                ADVFS_CRASH_RECOVERY_TEST(parentFtx.dmnP, AdvfsCrash_401);
    ...
    ADVFS_CRASH_RECOVERY_TEST(parentFtx.dmnP, AdvfsCrash_195);
    ftx_done_u( ftxH,
                FTA_IDX_UNDO_V1,
                sizeof(idxUndoRecordT),
                (void *) &undorec);
    ADVFS_CRASH_RECOVERY_TEST(parentFtx.dmnP, AdvfsCrash_196);
...
static statusT
idx_remove_filename_int(
    bs_meta_page_t      page_to_search,       /* node's page number */
    uint64_t            level,                /* level of b-tree calling */
    uint64_t            search_key,           /* value to search for */
    off_t               offset,               /* Uniquifier */
               ...
               (node_ptr->data[index].search_key == search_key))
        {
            ...
            if (node_ptr->data[index].loc.dir_offset == offset)
            {
                if (index < node_ptr->total_elements-1)
                {
                    ...
                    copy_node(&node_ptr->data[index+1],
                              &node_ptr->data[index],
                              (node_ptr->total_elements - index - 1));
	            ADVFS_CRASH_RECOVERY_TEST(ftxH.dmnP, AdvfsCrash_197);
                }
    ...
    )
{
     ...
     * or deletion will take place. If collisions cause us to
    ...
    while(!found)
    {
        if (index == node_ptr->total_elements)
        {
            ...
            if (sts != EOK)
            {
                /* If we run out of space attempting to snap the directory
        ...
        if (search_key == node_ptr->data[index].search_key)
        {
            ...
            if ((dir_rec_ptr->fs_dir_namecount
                 == filename_len) &&
                bcmp(dir_rec_ptr->fs_dir_name_string,
                 filename, filename_len) == 0)
            {
                GETTAG(tag,dir_rec_ptr);

                *found_bs_tag = tag;
                MS_SMP_ASSERT(dir_rec_ptr->fs_dir_bs_tag_num);
...
static statusT
idx_lookup_node_int(
    bs_meta_page_t      node_page,      /* btree node to search */
    uint32_t            level,          /* level of recursion */
    uint64_t            search_key,     /* hash vale to find */
    ...
    uint32_t            pin_flag,       /* indicates if returned node
                                           should be pinned */
    bs_meta_page_t      *page,          /* node page containing key */
    ftxHT               ftxH
    )
{
    ...
    if (sts != EOK)
    {
        /* If we ran out of space attempting to snap the index file
    ...
    )
{
    ...
    bs_meta_page_t root_page;
    uint32_t level;
    uint64_t location;          /* Both a page and a byte value */
    bfAccessT *idx_bfap;
    int split_count;
    idxNodeT *new_root;
    statusT sts;
    ...
    location = IDX_GET_ADDRESS(insert_page,insert_pgoff, idx_bfap);
    search_key = location;
    ...
    sts = FTX_START_N( FTA_NULL, &ftxH, parentFtx, idx_bfap->dmnP);
    if (sts != EOK) 
    {
    ...
    if (sts < 0 )
    {
         ...
         */
        ADVFS_CRASH_RECOVERY_TEST(ftxH.dmnP, AdvfsCrash_402);
        ftx_fail(ftxH);
        ADVFS_CRASH_RECOVERY_TEST(ftxH.dmnP, AdvfsCrash_403);
    ...
    if (sts==INSERT)
    {
        ...
        new_root->data[1].loc.node_page=location;
        new_root->data[0].search_key=search_key;
        ADVFS_CRASH_RECOVERY_TEST(ftxH.dmnP, AdvfsCrash_198);
        ...
        if (sts != EOK)
        {
            ...
            ADVFS_CRASH_RECOVERY_TEST(ftxH.dmnP, AdvfsCrash_199);
            ftx_done_n(ftxH,FTA_IDX_UNDO_V1);
            ADVFS_CRASH_RECOVERY_TEST(ftxH.dmnP, AdvfsCrash_200);
            return(sts);
        }
    }
    ...
    undorec.bfSetId = ((bfSetT *)idx_bfap->bfSetp)->bfSetId;
    undorec.dir_tag = dir_bfap->tag;
    undorec.nodeEntry.search_key = IDX_GET_ADDRESS(insert_page,unglom_offset, idx_bfap);
    ...
    ADVFS_CRASH_RECOVERY_TEST(ftxH.dmnP, AdvfsCrash_201);
    ftx_special_done_mode ( ftxH, FTXDONE_SKIP_SUBFTX_UNDO);
    ADVFS_CRASH_RECOVERY_TEST(ftxH.dmnP, AdvfsCrash_202);
    ftx_done_u( ftxH,
                FTA_IDX_UNDO_V1,
                sizeof(idxUndoRecordT),
                (void *) &undorec);
    ADVFS_CRASH_RECOVERY_TEST(ftxH.dmnP, AdvfsCrash_203);
...
static int
idx_directory_insert_space_int(
        bs_meta_page_t  node_page,      /* in - starting node */
        uint64_t        level,          /* level in the tree */
        uint64_t        *search_key,    /* in - key to insert */
                                        /* out - key to insert*/
        uint64_t        *size,          /* in - size to store*/
    ...
    )
{
    ...
    if (ret == INSERT)
    {
        ...
        else
        {
            ...
            node_ptr->data[index].loc.free_size = *size;
            ADVFS_CRASH_RECOVERY_TEST(ftxH.dmnP, AdvfsCrash_204);
...
idx_directory_get_space(
    ...
    off_t               *insert_pgoff,  /* out - offset where storage resides */
    ...
    )
{
    ...
    if (found_size > 0)
    {
        ...
        ADVFS_CRASH_RECOVERY_TEST(ftxH.dmnP, AdvfsCrash_205);
        ftx_done_u(ftxH,
                   FTA_IDX_UNDO_V1,
                   sizeof(idxUndoRecordT),
                   (void *)  &undorec);
        ADVFS_CRASH_RECOVERY_TEST(ftxH.dmnP, AdvfsCrash_206);
    }
    else
    {
        if (found_size == 0)
        {
            ...
            ADVFS_CRASH_RECOVERY_TEST(ftxH.dmnP, AdvfsCrash_207);
            ftx_done_n(ftxH,FTA_IDX_UNDO_V1);
            ADVFS_CRASH_RECOVERY_TEST(ftxH.dmnP, AdvfsCrash_208);
            return(TRUE);
        }
        else
        {
             ...
             * send the status up
             */
            ADVFS_CRASH_RECOVERY_TEST(ftxH.dmnP, AdvfsCrash_404);
            ftx_fail(ftxH);
            ADVFS_CRASH_RECOVERY_TEST(ftxH.dmnP, AdvfsCrash_405);
    ...
    if (found_size >  size)
    {
        ...
        sts=idx_directory_insert_space(dir_bfap,
                                       size,
                                       IDX_GET_PAGE_NO(insert_loc, idx_bfap),
                                       IDX_GET_OFFSET(insert_loc, idx_bfap),
                                       IDX_GET_OFFSET(insert_loc, idx_bfap),
                                       size,
                                       parentFtx);
    ...
    *insert_page=IDX_GET_PAGE_NO(location, idx_bfap);
    *insert_pgoff=IDX_GET_OFFSET(location, idx_bfap);
    ...
    )
{
    ...
    int next_node;
    uint32_t index;
    uint64_t required_size, free_size, rounded_free_size, requested_size;
    uint64_t start_addr;
    statusT sts;
    ...
    while ( TRUE )
    {
        ...
        if (node_ptr->data[index].loc.free_size >= requested_size)
        {
            ...
            free_size =  (uint64_t) node_ptr->data[index].loc.free_size;
            MS_SMP_ASSERT(free_size > 0);
            rounded_free_size =
                min(DIRBLKSIZ - (start_addr&(DIRBLKSIZ-1)), free_size);

            if ( required_size <= rounded_free_size )
            {
                *file_offset = start_addr;
                ...
                if ((rounded_free_size - required_size) < 
                    MIN_DIR_ENTRY_SIZE )
                {
                ...
                if(index < node_ptr->total_elements - 1)
                {
                    ...
                    ADVFS_CRASH_RECOVERY_TEST(ftxH.dmnP, AdvfsCrash_209);
                }
            ...
            if ((rounded_free_size != 0) &&
                (required_size <= rounded_free_size ))
            {
                *file_offset = start_addr;
                     ...
                     (node_ptr->page_right < 0)))
                {
                    ...
                    ADVFS_CRASH_RECOVERY_TEST(ftxH.dmnP, AdvfsCrash_210);
...
{
    int64_t bytes_to_trunc;
    bfAccessT *idx_bfap;
    uint64_t undo_space_start;
    statusT sts;
    idxUndoRecordT undorec;
    int32_t     partial_truncate = FALSE;
    ftxHT ftxH;
    int64_t ftx_started = FALSE;
    ...
    {	
        sts = FTX_START_N( FTA_NULL, &ftxH, parentFtxH, idx_bfap->dmnP);
        if (sts != EOK)
        {
            domain_panic(parentFtxH.dmnP,
    ...
    if (bytes_to_trunc < 0)
    {
        ...
        if (ftx_started) {
            ADVFS_CRASH_RECOVERY_TEST(parentFtxH.dmnP, AdvfsCrash_406);
            ftx_fail( ftxH );
            ADVFS_CRASH_RECOVERY_TEST(parentFtxH.dmnP, AdvfsCrash_407);
        }
    ...
    if (ftx_started) {
        ADVFS_CRASH_RECOVERY_TEST(parentFtxH.dmnP, AdvfsCrash_211);
        ftx_done_u( ftxH,
                    FTA_IDX_UNDO_V1,
                    sizeof(idxUndoRecordT),
                    (void *) &undorec);
        ADVFS_CRASH_RECOVERY_TEST(parentFtxH.dmnP, AdvfsCrash_212);
    }

    if (!partial_truncate) {
        /*
         * If we didn't truncate the entire amount, then leave the flag set
         * to truncate later.
         */
        ((bsIdxRecT *)idx_bfap->idx_params)->flags &= ~(IDX_TRUNCATE);
    }
    ...
    )
{
    ...
    {
        domain_panic(ftxH.dmnP,
                     "idx_setup_for_truncation_int: rbf_pinpg failed");
        return(sts);
    }
    ...
    else
    {
        ...
        if (node_ptr->total_elements == 0)
        {
            ...
            if (page_to_search < 0) {
                 ...
                 * should just fail the truncation.
                 */
                MS_SMP_ASSERT(operation == TRUNC_QUERY);
                (void) bs_derefpg(bfpgref,BS_CACHE_IT);
                return(0);
            }
            MS_SMP_ASSERT(node_ptr->page_right < 0);
        ...
        if (num_bytes < ADVFS_METADATA_PGSZ)
        {
            MS_SMP_ASSERT(operation == TRUNC_QUERY);
            bs_derefpg(bfpgref,BS_CACHE_IT);
        ...
        if ( ((num_bytes & (ADVFS_METADATA_PGSZ - 1)) > 0) || 
             ( node_ptr->data[index].search_key < 
               (uint64_t)dir_bfap->file_size ) )
        {
            /* This is not full pages leave the remainder and remove
             * the rest or the size of the truncate has been limited due
             * to snapshots.
             */
            ...
            if (operation == TRUNC_UPDATE)
            {
                uint64_t remainder;
                /*
                 * The truncate of the directory may have been limited
                 * because of snapshots.  Make sure we only truncate
                 * to the size of the dir_bfap and not smaller than
                 * the current size.
                 */
                MS_SMP_ASSERT( (dir_bfap->file_size % 
                                (dir_bfap->bfPageSz * ADVFS_FOB_SZ)) == 0 );

                /* Due to transaction constrants with snapshots, the
                 * size of the truncation may be limited. This is
                 * accomplished by setting the filesize to limit the
                 * truncation and then leaving the IDX_TRUNCATE flag
                 * set so that the next insertion will trigger more
                 * truncation.
                 */

                remainder = dir_bfap->file_size - 
                    node_ptr->data[index].search_key;

                if (remainder >= ADVFS_METADATA_PGSZ) {
                    /*
                     * Indicate to caller that the truncate was limited and
                     * the IDX_TRUNCATE flag should not be cleared 
                     */
                    SNAP_STATS( idx_trunc_limited_truncate );
                    *partial_truncate = TRUE;

                } else {
                    MS_SMP_ASSERT((num_bytes & 
                                   (ADVFS_METADATA_PGSZ - 1)) == remainder);
                }
                /*
                 * Figure out the actual number of bytes to truncate
                 * based on the directory's file size and the offset of
                 * the free space.
                 */
                num_bytes -= remainder;
                ...
                /*
                 * Leave whatever free_size is not being truncated.
                 */
                node_ptr->data[index].loc.free_size = remainder;
                MS_SMP_ASSERT(node_ptr->data[index].loc.free_size >=
                              MIN_DIR_ENTRY_SIZE);
            ...
            else
            {
                ...
                num_bytes -= (num_bytes & (ADVFS_METADATA_PGSZ - 1));
...
idx_glom_entry_int(
    ...
    uint64_t     start_addr,    /* free space start    */
    uint64_t     size,          /* free space size     */
    ...
    )
{
         ...
         left_node_ptr->data[left_index].loc.free_size >= start_addr))
    {
        ...
        MS_SMP_ASSERT(left_node_ptr->data[left_index].search_key <= 
                      start_addr);
    ...
    if (reuse_ptr != NULL)
    {
            ...
            <= size))
        {
            ...
            if (right_index < right_node_ptr->total_elements - 1)
            {
                ...
                copy_node(&right_node_ptr->data[right_index+1],
                          &right_node_ptr->data[right_index],
                          (right_node_ptr->total_elements - right_index - 1));
            
                ADVFS_CRASH_RECOVERY_TEST(ftxH.dmnP, AdvfsCrash_213);
            }
        ...
        if ((reuse_ptr->loc.free_size >= ADVFS_METADATA_PGSZ ) &&
            (reuse_ptr->search_key +
    ...
    if ( right_node_ptr->data[right_index].search_key - start_addr <= size )
    {
        ...
        if ((right_node_ptr->data[right_index].loc.free_size 
             >= ADVFS_METADATA_PGSZ) &&
            (right_node_ptr->data[right_index].search_key +
             right_node_ptr->data[right_index].loc.free_size ==
             dir_bfap->file_size))
    ...
    )
{
    ...
    uint64_t last_freespace_end;
    uint64_t pg_off;
    fs_dir_entry *dir_p, *first_p;
    int n;
    char *p, *dir_page_end;
    statusT sts;
    ...
    dir_page_end = ((char *)dir_p + ADVFS_METADATA_PGSZ);
    ...
    while ((char *)dir_p < dir_page_end) {
        ...
        if (dir_p->fs_dir_bs_tag_num == 0)
        {
                ...
                (last_freespace_end == pg_off))
            {
                ...
                freespace_node->data[free_idx].loc.free_size +=
                    dir_p->fs_dir_size;
                last_freespace_end += dir_p->fs_dir_size;
                free_idx++;
            }
            else
            {
        ...
        if ( FS_DIR_SIZE_OK(dir_p->fs_dir_size, pg_off)) {
            /* there has been a minor data corruption, inform and error out */
            fs_dir_size_notice(dir_bfap, n, (long)0, (uint_t)pg_off);    
            /*
             * Resume processing on the next directory block boundary.
             */

            n = (DIRBLKSIZ + pg_off) & ~(DIRBLKSIZ-1);
            p = (char *)first_p;
            pg_off = 0;
        }
    ...
    ADVFS_CRASH_RECOVERY_TEST(ftxH.dmnP, AdvfsCrash_214);
    ...
    RBF_PIN_FIELD(free_pgref,freespace_node->total_elements);
    ADVFS_CRASH_RECOVERY_TEST(ftxH.dmnP, AdvfsCrash_215);
    ...
    )
{
    ...
    uint64_t search_key;
    uint32_t levels;
    bs_meta_page_t old_root;
    bs_meta_page_t new_root;
    bs_meta_page_t pg_cnt;
    bsIdxRecT *idx_params;
    uint32_t i;
    void * msg_array;
    ftxHT ftxH;
    ...
    for(i=FNAME;i<=FFREE;i++)
    {
        ...
        finish_prune:
            ADVFS_CRASH_RECOVERY_TEST(ftxH.dmnP, AdvfsCrash_216);
            ftx_special_done_mode ( ftxH, FTXDONE_SKIP_SUBFTX_UNDO);
            ADVFS_CRASH_RECOVERY_TEST(ftxH.dmnP, AdvfsCrash_217);
            ftx_done_n( ftxH,FTA_IDX_UNDO_V1);
            ADVFS_CRASH_RECOVERY_TEST(ftxH.dmnP, AdvfsCrash_218);
    ...
    )
{
    ...
    bs_meta_page_t new_root_page;
    int32_t copy_cnt;
    uint32_t status, redistribute;
    bs_meta_page_t right_page_right, right_page_left;
    statusT sts;
        ...
        if(adjust_range == REMOVE)
        {
            ...
            if (index_to_remove <= dst_node_ptr->total_elements-1)
            {
                 ...
                 if (index_to_remove < dst_node_ptr->total_elements-1)
                 {
                     ...
                     copy_node(&dst_node_ptr->data[index_to_remove+1],
                               &dst_node_ptr->data[index_to_remove],
                               (dst_node_ptr->total_elements-index_to_remove-1));
            
                     ADVFS_CRASH_RECOVERY_TEST(ftxH.dmnP, AdvfsCrash_219);
                 }
             ...
             else
             {
                      ...
                      right_node_ptr->total_elements))
                 {
                   ...
                   RBF_PIN_FIELD(dst_pgref,dst_node_ptr->data[index].search_key);
                     ...
                     if (copy_cnt != 1)
                     {
                         copy_node(&right_node_ptr->data[1],
                     ...
                     if (copy_cnt > 0)
                     {
                         copy_node(&dst_node_ptr->data[dst_node_ptr->total_elements 
                                                       - copy_cnt],
                                   &right_node_ptr->data[0],
                                   copy_cnt);
                   ...
                   *search_key = 
                   dst_node_ptr->data[dst_node_ptr->total_elements-1].search_key;
                     ADVFS_CRASH_RECOVERY_TEST(ftxH.dmnP, AdvfsCrash_220);
                     return(MODIFY);
                 }
         ...
         if (copy_cnt > 0)
         {
             ...
             copy_node(&right_node_ptr->data[copy_index],
                       &dst_node_ptr->data[dst_node_ptr->total_elements],
                       copy_cnt);

             dst_node_ptr->total_elements += copy_cnt;
             MS_SMP_ASSERT(dst_node_ptr->total_elements <= IDX_MAX_ELEMENTS);
             ADVFS_CRASH_RECOVERY_TEST(ftxH.dmnP, AdvfsCrash_221);
         }
         ...
         if (redistribute)
         {
             ...
             RBF_PIN_FIELD(right_pgref,right_node_ptr->total_elements);
             copy_node(&right_node_ptr->data[copy_cnt+copy_index],
                       &right_node_ptr->data[0],
                       right_node_ptr->total_elements -  
                       (copy_cnt + copy_index));
             ...
             MS_SMP_ASSERT(dst_node_ptr->total_elements <= IDX_MAX_ELEMENTS);
             ADVFS_CRASH_RECOVERY_TEST(ftxH.dmnP, AdvfsCrash_222);
    ...
    if (adjust_range == REMOVE)
    {
        ...
        if (index_to_remove < dst_node_ptr->total_elements - 1)
        {
            ...
            rbf_pin_record(right_pgref,
                           ...
                           sizeof(idxNodeEntryT));
            copy_node(&right_node_ptr->data[index_to_remove+1],
                      &right_node_ptr->data[index_to_remove],
                      (right_node_ptr->total_elements-index_to_remove - 1));
            
            ADVFS_CRASH_RECOVERY_TEST(ftxH.dmnP, AdvfsCrash_223);
        }
    ...
    )
{
    ...
    sts = stg_remove_stg_start (idx_bfap,
                                page_to_remove * idx_bfap->bfPageSz,
                                idx_bfap->bfPageSz,
    ...
    /* Let go of the lock then adjust the quotas */

    sts = chk_blk_quota( &((bsIdxRecT *)idx_bfap->idx_params)->dir_bfap->bfVnode,
    ...
    )
{
    ...
    while(pmsgs->msg_cnt > 0)
    {
        ...
        if (msg) {
            ...
            msg->ahm_data.ahm_dtinfo.delCnt = pmsgs->msgs[pmsgs->msg_cnt-1].delCnt;
            msg->ahm_data.ahm_dtinfo.delList = pmsgs->msgs[pmsgs->msg_cnt-1].delList;
            msg->ahm_data.ahm_dtinfo.domain.id = idx_bfap->dmnP->domainId;
    ...
    )
{
    ...
    if (sts != EOK) {
        domain_panic(ftxH.dmnP,"idx_undo_opx: bfs_open failed");
        return;
    }
    ...
    case IDX_REMOVE_FILENAME_UNDO :
        ...
        sts=idx_insert_filename_int(
            ...
            ((bsIdxRecT *)idx_bfap->idx_params)->bmt.fname_levels,
            &recp.nodeEntry.search_key,
            (off_t*)&recp.nodeEntry.loc.dir_offset,
            idx_bfap,
            &insert,
     ...
     * last close on the directory then bs_close will call advfs_inactive
    ...
    sts = bs_close(dir_bfap, MSFS_CLOSE_NONE);
    ...
    if (sts != EOK) {
        domain_panic(ftxH.dmnP,"bs_bfs_close error in idx_undo_opx");
        return;
    }
}
    ...
    )
{
    ...
    rbfPgRefHT pgref,right_pgref;
    uint32_t index;
    uint64_t new_search_key;
    uint64_t start_addr, entry_sa;
    uint64_t size,entry_sz;
    bs_meta_page_t node_page;
    statusT sts;
        ...
        if(entry_sz > size)
        {
            ...
            if(index < node_ptr->total_elements)
            {
                ...
                rbf_pin_record(pgref,&node_ptr->data[index],
                               (node_ptr->total_elements - index) *
                               sizeof(idxNodeEntryT));
                ADVFS_CRASH_RECOVERY_TEST(ftxH.dmnP, AdvfsCrash_224);
            }
    ...
    else
    {
        ...
        if(index < node_ptr->total_elements - 1)
        {
            ...
            copy_node(&node_ptr->data[index+1],
                      &node_ptr->data[index],
                      (node_ptr->total_elements - index - 1));
            ADVFS_CRASH_RECOVERY_TEST(ftxH.dmnP, AdvfsCrash_225);
        }
...
/* This replaces calls to bcopy. On Tru64 the bcopy handles
 * the overlapping case. On hp/ux it does not.  
 */
...
copy_node(	idxNodeEntryT *src,
                ...
                int32_t cnt)
{
    /* Figure out if we are rippling down or up
     * and copy the correct direction. If we aren't
     * rippling then either direction will work
     */
    int32_t i;
    cnt = cnt - 1;
    if (src < dst){
