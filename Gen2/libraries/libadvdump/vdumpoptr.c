	...
	char	       *question;
{
	...
	{
		fprintf(stderr,catgets(catd, RVDUMP, FOPENTTY, 
	...
	for (;;)
	{
		...
		fprintf(stderr,catgets(catd, RVDUMP, YESNO1, 
                    "\"%s\" or \"%s\"?\n\n"), yes_str, no_str);
...
alarmcatch()
{
	...
	if (notify_flag == TRUE)
	{
		broadcast(catgets(catd, RVDUMP, NEEDATTB, 
...
pose_question()
{
	...
	fprintf(stderr,catgets(catd, RVDUMP, NEEDATT1, 
	...
	char	       *message;
{
	...
	{
	case -1:
		fprintf(stderr,catgets(catd, RVDUMP, CNTFK, 
	...
	if (errno == ENOENT || errno == EACCES) {
	    fprintf(stderr, catgets(catd, RVDUMP, COUTMP,
	...
	char	       *tty_name, *message;
{
	...
	if ((tty_fp = fopen(tty_path, "w")) != NULL)
	{
		...
		(void) fprintf(tty_fp, catgets(catd, RVDUMP, MFD1, 
                    "Message from the dump program to all operators "));
		(void) fprintf(tty_fp, catgets(catd, RVDUMP, MFD2, 
                    "at %d:%02d ... \r\n\n"), 
