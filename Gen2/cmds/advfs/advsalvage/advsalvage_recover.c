                 ...
                 filesetLLT  *filesetp)                
{
    ...
    recoverData.fsId = filesetp->filesetId.dirTag.tag_num;
     ...
     * exceeded.
     */
    fprintf (Options.logFD, catgets(mcat, S_SALVAGE_1, SALVAGE_167, 
             "\n    Recovery quotas for '%s':\n"), filesetp->fsName);

    if (NULL != filesetp->quota)
    {
        fprintf (Options.logFD, catgets(mcat, S_SALVAGE_1, SALVAGE_168, 
                 "Hard block limit = %lu, soft block limit = %lu\n"),
                 filesetp->quota->hardBlockLimit, filesetp->quota->softBlockLimit);
        fprintf (Options.logFD, catgets(mcat, S_SALVAGE_1, SALVAGE_169, 
    ...
    if (FAILURE == status)
    {
        fprintf (Options.logFD, catgets(mcat, S_SALVAGE_1, SALVAGE_170, 
                 "Could not complete recovery of file system/snapshot\n"));
        writemsg (SV_ERR, catgets(mcat, S_SALVAGE_1, SALVAGE_171, 
                  "Could not complete recovery of file system/snapshot '%s'\n"), 
                  filesetp->fsName);
    }
    else
    {
    ...
    if (FAILURE != status)
    {
        writemsg (SV_VERBOSE, catgets(mcat, S_SALVAGE_1, SALVAGE_172, 
                  "Recovery of '%s'completed, check log and error files\n"), 
                  filesetp->fsName);
    }

    fprintf (Options.logFD,catgets(mcat, S_SALVAGE_1, SALVAGE_173, 
             "%lu files processed, including %lu directories\n"),
             recoverData.totalFilesFound, recoverData.dirFiles);
    fprintf (Options.logFD,catgets(mcat, S_SALVAGE_1, SALVAGE_174, 
             "%lu successfully recovered, %lu partially recovered\n"),
             recoverData.successFiles, recoverData.partialFiles);
    fprintf (Options.logFD,catgets(mcat, S_SALVAGE_1, SALVAGE_175, 
             "%lu recovered larger, %lu unrecovered\n"),
             recoverData.largerFiles, recoverData.unrecoveredFiles);
    fprintf (Options.logFD,catgets(mcat, S_SALVAGE_1, SALVAGE_176, 
             "%lu had no attributes.\n"), recoverData.noAttributeFiles);
    if (Options.outputFormat == F_TAR) {
        fprintf (Options.logFD,catgets(mcat, S_SALVAGE_1, SALVAGE_190, 
                "%lu files lost ACLs during pax archive output.\n"), 
                recoverData.lost_acls);
    }
                  ...
                  recoverDataT     *recoverData)
{
	...
	if ((fileCounter % 1000) == 0) {
	    writemsg(SV_ALL | SV_CONT, 
		     catgets(mcat, S_SALVAGE_1, SALVAGE_284, 
			     "Recovering file %ld out of %ld.\r"),
        ...
        || (0 != Options.recPartial))
    {
        ...
        if (S_IS_RECOVERED (tagEntryp->status))
        {
            ...
            if (SUCCESS != status)
            {
                ...
                fprintf (Options.logFD, catgets(mcat, S_SALVAGE_1, SALVAGE_177, 
                         "%s : %ld : hardlink not created\n"), recoverData->pathName, 
                         nodep->tagNum);
                writemsg (SV_ALL, catgets(mcat, S_SALVAGE_1, SALVAGE_178, 
            ...
            else
            {
                ...
                {
                    fprintf (Options.logFD, catgets(mcat, S_SALVAGE_1, 
                             SALVAGE_179, "%s : %ld : hardlink created\n"), 
                             recoverData->pathName, nodep->tagNum);
                }
                writemsg (SV_VERBOSE, catgets(mcat, S_SALVAGE_1, SALVAGE_180, 
                          "Hardlink '%s' created\n"), recoverData->pathName);
        ...
        if (0 == status)
        {
            if (-1 == check_overwrite (recoverData->pathName))
            {
                ...
                writemsg (SV_VERBOSE, catgets(mcat, S_SALVAGE_1, SALVAGE_182, 
            ...
            else
            {
                ...
                if (retStatus < 0 || syscallStatus < 0)
                {
                    fprintf (Options.logFD, catgets(mcat, S_SALVAGE_1, 
                             ...
                             recoverData->pathName, nodep->tagNum);
                    writemsg (SV_ALL, catgets(mcat, S_SALVAGE_1, SALVAGE_184, 
        ...
        {
            recoverData->unrecoveredFiles++;
            fprintf (Options.logFD, catgets(mcat, S_SALVAGE_1, SALVAGE_185, 
                     "%s : %ld : could not be restored\n"), recoverData->pathName, 
                     nodep->tagNum);
            writemsg (SV_ALL, catgets(mcat, S_SALVAGE_1, SALVAGE_186, 
        ...
        else
        {
            ...
            if (SUCCESS == status)
            {
                ...
                writemsg (SV_VERBOSE, catgets(mcat, S_SALVAGE_1, SALVAGE_188, 
            ...
            {
                recoverData->partialFiles++;
                writemsg (SV_NORMAL, catgets(mcat, S_SALVAGE_1, SALVAGE_189, 
            ...
            /* 
             * If we have ACLs for the file, restore them
             */
            if (!S_IS_ACL_NONE (tagEntryp->status))
            {
                status = SUCCESS;
                if (S_IS_ACL_COMPLETE (tagEntryp->status))
                {
                    struct timeval fileTimes[2];  /* time values for utimes call */
                    
                    status = restore_acls (tagEntryp, 
					   recoverData->pathName,
                                           localPath);
                    /*
                     * If the attributes exist, then reset the times after
                     * restoring the ACLs, since restoring the
                     * ACLs changes them.
                     */
                    if (NULL != tagEntryp->attrs)
                    {
                        ...
                        if (-1 == utimes (localPath, fileTimes))
                        {
                            fprintf (Options.logFD, catgets(mcat, S_SALVAGE_1, 
                                 SALVAGE_211, 
                                 "%s: cannot restore file atime/mtime\n"), 
                                 recoverData->pathName);
                        }
                    }
                }
                
                if (S_IS_ACL_PARTIAL (tagEntryp->status) || (SUCCESS != status))
                {
                    if (NO_SPACE == status)
                    {
                        status = FAILURE;
		    ...
		    else
		    {
                    ...
                    fprintf (Options.logFD, catgets(mcat, S_SALVAGE_1, 
                             SALVAGE_79, 
                             "%s : %ld : ACLs not restored\n"),
                             recoverData->pathName, nodep->tagNum);
                    writemsg (SV_ALL, catgets(mcat, S_SALVAGE_1, SALVAGE_80,
                              "ACLs not restored for file '%s'\n"),
                              recoverData->pathName);
                } /* end if partial ACL buffer */
            } /* end if ACLs exist */
			  ...
			  recoverDataT     *recoverData)
{
	...
	if ((S_IS_PARTIAL (tagEntryp->status)) && (0 != Options.addSuffix))
	{
	    ...
	    if (NULL == partialName)
	    {
		writemsg (SV_ERR, catgets(mcat, S_SALVAGE_1, SALVAGE_198, 
	    ...
	    strncat (partialName, catgets(mcat, S_SALVAGE_1, SALVAGE_199, 
					  ".partial"), 9);
	...
	if (strlen(recoverData->pathName) > PATH_MAX - 1)
	{
	    ...
	    fprintf(Options.logFD, catgets(mcat, S_SALVAGE_1,
		    ...
		    recoverData->pathName, nodep->tagNum);
	    writemsg(SV_ALL, catgets(mcat, S_SALVAGE_1,
	...
	if (NULL == tagEntryp->attrs)
	{
	    ...
	    fprintf(Options.logFD, catgets(mcat, S_SALVAGE_1, SALVAGE_204, 
		    "%s: %ld : attributes were not found\n"),
		    recoverData->pathName, nodep->tagNum);
	    writemsg(SV_VERBOSE, catgets(mcat, S_SALVAGE_1, SALVAGE_205, 
	...
	/*
	 * If we are in test mode, and not running as root, set the ownership
	 * to the current effective ownership
	 */
	if (Options.qtest && geteuid()) {
	    statbuf.sb_uid = geteuid();
	    statbuf.sb_gid = getegid();
	}
	...
	switch (tagEntryp->fileType)
	{
		...
		if (NULL == tagEntryp->addAttrs)
		{
		    fprintf(Options.logFD, catgets(mcat, S_SALVAGE_1,
			    SALVAGE_214, 
			    "%s: %ld : symlink has no name to link to\n"),
			    recoverData->pathName, nodep->tagNum);
		    statbuf.sb_link[0] = '\0';
		    statbuf.sb_size = 0;
		}
	...
	/* 
	 * If we have a ACLs for the file, restore them
	 */
	if (!S_IS_ACL_NONE (tagEntryp->status))
	{
            /* 
             * ACLs can't currently be stored in a pax archive. Log this fact
             * and move on.
             */
            recoverData->lost_acls++;
            fprintf (Options.logFD, catgets(mcat, S_SALVAGE_1,
                 ...
                 char        *localPath)
{
    ...
    tagNumT tagNum;        /* tag number for log messages */
    ...
    if (status != 0)
    {
        do
        {
            ...
            if (0 != status)
            {
                ...
                if (ENOSPC == errno)
                {
                    writemsg (SV_ERR, catgets(mcat, S_SALVAGE_1, SALVAGE_192, 
                              "No room to create file '%s'\n"), pathName);
                    writemsg (SV_ERR | SV_CONT, catgets(mcat, S_SALVAGE_1, SALVAGE_193, 
                             "Add more space to file system and continue or abort\n"));
                    if (want_abort ())
                    {
                        return NO_SPACE;
                    }
    ...
    if (NULL != tagEntryp->attrs)
    {
        ...
        {
            fprintf (Options.logFD, catgets(mcat, S_SALVAGE_1, SALVAGE_194, 
                     "%s: %ld : Error setting file attributes\n"), 
                     pathName, tagNum);
            writemsg (SV_NORMAL, catgets(mcat, S_SALVAGE_1, SALVAGE_195, 
                      "Error setting attributes for file '%s'\n"), pathName);
        }
    }
    else
    {
        fprintf (Options.logFD, catgets(mcat, S_SALVAGE_1, SALVAGE_196, 
                 "%s: %ld : No attributes found for directory\n"), 
                 pathName, tagNum);
        writemsg (SV_VERBOSE, catgets(mcat, S_SALVAGE_1, SALVAGE_197, 
...

/*
 * Function name: restore_reg_file
 *
 * Description:
 *
                      ...
                      volMapT     *volumes)
{
    ...
    if ((S_IS_PARTIAL (tagEntryp->status)) && (0 != Options.addSuffix))
    {
        ...
        {
            writemsg (SV_ERR, catgets(mcat, S_SALVAGE_1, SALVAGE_267,
        ...
        if (-1 == fd)
        {
            ...
            if (ENOSPC == errno)
            {
                ...
                writemsg (SV_ERR | SV_CONT, 
                          catgets(mcat, S_SALVAGE_1, SALVAGE_193, 
                          "Add more space to file system and continue or abort\n"));
                if (want_abort ())
                {
                    return NO_SPACE;
        ...
        if (PARTIAL == status)
        {
            ...
            else
            {
                ...
                if ((0 != Options.addSuffix) && 
                    (NULL == strstr (*localPathp, catgets(mcat, S_SALVAGE_1, 
                                     SALVAGE_199, ".partial"))))
                {
                            ...
                            if (-1 == status)
                            {
                                ...
                                if (ENOSPC == errno)
                                {
                                    writemsg (SV_ERR, catgets(mcat, S_SALVAGE_1, 
                                              ...
                                              pathName);
                                    writemsg (SV_ERR | SV_CONT, catgets(mcat, S_SALVAGE_1,
                                      SALVAGE_193, 
                                      "Add more space to file system and continue or abort\n"));
                                    if (want_abort ())
                                    {
    ...
    if (NULL != tagEntryp->attrs)
    {
        if (SUCCESS != (status = restore_attrs (pathName, *localPathp, 
                                                tagEntryp->attrs)))
        {
            fprintf (Options.logFD, catgets(mcat, S_SALVAGE_1, SALVAGE_203, 
                     "%s : %ld : Error setting file attributes\n"), 
                     pathName, tagNum);
            writemsg (SV_NORMAL, catgets(mcat, S_SALVAGE_1, SALVAGE_195, 
                     "Error setting attributes for file '%s'\n"), pathName);
    ...
    if (S_IS_PARTIAL (tagEntryp->status))
    {
        ...
        fprintf (Options.logFD, catgets(mcat, S_SALVAGE_1, SALVAGE_206, 
                 "%s: %ld : partially recovered\n"), pathName, tagNum);
        ...
        while (NULL != extentp)
        {
            if (-2 == extentp->diskBlock)
            {
                fprintf (Options.logFD, catgets(mcat, S_SALVAGE_1, SALVAGE_207, 
...

/*
 * Function name: restore_file_data
 *
 * Description:
                       ...
                       volMapT     *volumes)
{
    ...
    int64_t   bytePosition;   /* current byte position in file */
    int64_t   extSize;        /* current extent size */
    bf_vd_blk_t extLBN;       /* current LBN for extent */
    int64_t   readLength;     /* number of bytes for read operation */
    ...
    while (NULL != extentp)
    {
	    ...
	    int64_t bytesRead;
            ...
            if (extSize < ADVFS_METADATA_PGSZ)
            {
                readLength = DEV_BSIZE * (extSize/DEV_BSIZE + (extSize%DEV_BSIZE?1:0));
            ...
            {
                readLength = ADVFS_METADATA_PGSZ;
		...
		switch (Options.outputFormat)
		{
			...
			do
			{
				...
				if (ENOSPC == errno)
				{
				    ...
				    writemsg (SV_ERR | SV_CONT, catgets(mcat,
			...
			tmpByteInFile = extentp->byteInFile;
                    ...
                    else if (extLBN == extentp->diskBlock)
                    {
                        ...
                        else
                        {
                             ...
                             */
                            newExtp = NULL;
                            status = create_extent_element (&newExtp, -2, -2,
				...
				case F_TAR:
				    ...
				    break;
				default:
				    writemsg(SV_DEBUG, catgets(mcat,
                    ...
                    else if (extSize - readLength <= 0)
                    {
                         ...
                         * page to it.
                         */
                        if (NULL != extentp->next && -2 == extentp->next->diskBlock)
                        {
			    ...
			    goto _return;
                        ...
                        extentp->diskBlock += newExtp->extentSize/DEV_BSIZE 
                                              + readLength/DEV_BSIZE;
 ...
 _return:
	...
	{
	    writemsg(SV_DEBUG, catgets(mcat, S_SALVAGE_1, SALVAGE_264,
		   ...
		   tagAttributeT *attrp)
{
	...
	{ 
	    fprintf (Options.logFD, catgets(mcat, S_SALVAGE_1, SALVAGE_209, 
		     "%s: cannot restore uid/gid\n"), pathName);
	    status = FAILURE;
	}
    }
    if (-1 == chmod (localPath, attrp->mode & 07777))
    {
        fprintf (Options.logFD, catgets(mcat, S_SALVAGE_1, SALVAGE_210, 
                 "%s: cannot restore file permissions\n"), pathName);
        status = FAILURE;
    }
    ...
    if (-1 == utimes (localPath, fileTimes))
    {
        fprintf (Options.logFD, catgets(mcat, S_SALVAGE_1, SALVAGE_211, 
                 "%s: cannot restore file atime/mtime\n"), pathName);
        status = FAILURE;
    }
    return status;
...

/*
 * Function name: restore_acls
 *
 * Description:
 *  This routine restores any ACLs for a file.
 *
 * Input parameters: 
 *  tagEntryp: pointer to the data for the tag for this file.
...
{
    int retry;          /* retry flag for out of space failure */
    int aBufSize;       /* total size in bytes of ACL buffer */
    int status;         /* returned status from acl() call */
    struct acl *buffer; /* pointer the acl buffer */
    int i;

    assert (sizeof(struct acl) == sizeof(advfs_acl_t));
    /*
     * The caller should make sure that the ACL buffer is 
     * complete, but just in case
     */
    if (!S_IS_ACL_COMPLETE(tagEntryp->status)) {
        return FAILURE;
    }

    aBufSize = tagEntryp->num_acls * sizeof(struct acl);
    buffer = (struct acl *)tagEntryp->acls;
	...
	{
	    case F_DISK:
		status = acl (localPath, ACL_SET, tagEntryp->num_acls, buffer);
		break;
	    case F_TAR:
                /* 
                 * We can't currently store ACLs in a pax archive so ignore
                 * this for now.
                 */
		break;
	    default:
		     ...
		     char        *localPath)
{
    ...
    if (NULL == tagEntryp->addAttrs)
    {
        fprintf (Options.logFD, catgets(mcat, S_SALVAGE_1, SALVAGE_214, 
                 "%s: %ld : symlink has no name to link to\n"), pathName, tagNum);
        return FAILURE;
    }
    ...
    if (NULL != tagEntryp->attrs)
    {
        if (-1 == lchown (localPath, tagEntryp->attrs->uid, 
                          tagEntryp->attrs->gid))
        {
            fprintf (Options.logFD, catgets(mcat, S_SALVAGE_1, SALVAGE_215, 
                     "%s: %ld : cannot set uid/gid for symlink\n"), pathName, tagNum);
            writemsg (SV_NORMAL, catgets(mcat, S_SALVAGE_1, SALVAGE_216, 
                      "Error restoring attributes for file '%s'\n"), pathName);
        }
    }
    else
    {
        fprintf (Options.logFD, catgets(mcat, S_SALVAGE_1, SALVAGE_217, 
                 "%s: %ld : no attributes for symlink\n"), pathName, tagNum);
        writemsg (SV_VERBOSE, catgets(mcat, S_SALVAGE_1, SALVAGE_205, 
                  "Attributes not found for file '%s'\n"), pathName);
    }
    return SUCCESS;
                     ...
                     filesetTagT      *tagEntryp)
{
    ...
    {
        fprintf (Options.logFD, catgets(mcat, S_SALVAGE_1, SALVAGE_218, 
                 "%s: %ld : No attributes available for file\n"), 
                 pathName, nodep->tagNum);
        writemsg (SV_VERBOSE, catgets(mcat, S_SALVAGE_1, SALVAGE_205, 
                  "Attributes not found for file '%s'\n"), pathName);
    }
...

/*
 * Function name: restore_hardlink
 *
 * Description:
        ...
        if (NULL != tagEntryp->attrs)
        {
            ...
            if (SUCCESS == status)
            {
                ...
                if (-1 == utimes (localPath, fileTimes))
                {
                    fprintf (Options.logFD, catgets(mcat, S_SALVAGE_1, 
                             SALVAGE_219, 
                             "%s: %ld :cannot reset directory atime/mtime\n"),
                             pathName, nodep->tagNum);
                }
...
int output_tar_data (int64_t fileSize, int64_t bufSize, int zeroFill,
		     char *buffer, int64_t *bytesWritten)
{
    char *funcName = "output_tar_data";
    uint64_t avail;	 /* Amount of free space remaining in tar buffer */
    char     *tarbuf;	 /* Pointer into the tar buffer for writing data */
    uint64_t chunk;	 /* Amount of data to write to archive this pass */
    int64_t i;
	...
	else
	{
	    if (buffer == NULL)
	    {
		writemsg(SV_DEBUG, catgets(mcat, S_SALVAGE_1, SALVAGE_263,
