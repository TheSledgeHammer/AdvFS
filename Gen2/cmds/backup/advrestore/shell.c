...
do_chdir( int dir_fd, char *arg )
{
    ...
    if (ret != OKAY) {
        fprintf( stderr, catgets(catd, S_SHELL1, SHELL1, 
...
ls( int dir_fd, char *path, int list_all )
{
	...
	if (num_files != 0) {
	    ...
	    if (files == NULL) {
	      fprintf(stderr, catgets(catd, S_SHELL1, SHELL31, 
                  "out of memory; cannot list files\n"));
    ...
    else
    {
        ...
        if (ino == 0) {
            fprintf( stderr, catgets(catd, S_SHELL1, SHELL2, 
    ...
    if (User_Quota_data_present && ino == Root_ino) {
        ...
        if (list_all || restore_uquota) {
            ...
            if (files[i].name == NULL) {
                fprintf(stderr, catgets(catd, S_SHELL1, SHELL4, 
                    "out of memory; cannot list all files\n"));
                free(files);
                return;
            }
            files[i].name[0] = ' ';
        ...
        if (s_len > 0 && (cmd = strtok( sp, " " )) != NULL) {
            ...
            if (strcmp( "ls", cmd ) == 0) {
                ...
                *verbose ^= 1;
                ...
                fprintf( stderr, catgets(catd, S_SHELL1, SHELL30, 
                    "unknown command\n") );
            }
        }
