...
char Cwd_name[MAXPATHLEN+1];    /* current working dir's full path name */
    ...
    )
{
    ...
    snprintf( Tmp_dname, sizeof(Tmp_dname) - 1, "%s.%d", dir_path, getpid() );

    dir_fd = open( Tmp_dname, O_CREAT | O_RDWR | O_EXCL, 0640 );
    if (dir_fd < 0) {
        fprintf( stderr, catgets(catd, S_DIRMGT1, DIRMGT2, 
            "%s: cannot create restore dir <%s>; [%d] %s; terminating\n"),
    ...
    )
{
    if (write( dir_fd, buf, bytes ) < bytes) {
        fprintf( stderr, catgets(catd, S_DIRMGT1, DIRMGT3, 
            "%s: dir_write() error; [%d] %s; terminating\n"),
            Prog, errno, ERR_MSG );
        exit( 1 );
    }
    ...
    )
{
    ...
    Cur_dir_mk = ino_tbl_add_dir( dir_stat->st_ino, 
                                  dir_path, 
                                  tell( dir_fd ), 
                                  0,
                                  parent_ino );
    ...
    )
{
    ...
    if (pos < 0) {
        fprintf( stderr, 
                 catgets(catd, S_DIRMGT1, DIRMGT4, 
                 "%s: dir_access() lseek error; [%d] %s; terminating\n"),
                 Prog, errno, ERR_MSG );
        exit( 1 );
    }
    ...
    while (TRUE) {
        ...
        if ((dir_desc->buf_offset + MIN_ENT_SZ) >= DIRBLKSIZ) {
            ...
            if (pos != dir_desc->cur_offset) {
                fprintf( stderr, catgets(catd, S_DIRMGT1, DIRMGT5, 
                    "%s: dir_read() seek error; [%d] %s; terminating\n"),
                    Prog, errno, ERR_MSG );
                exit( 1 );
            }
		...
		if (rcnt == 0) {
                    ...
                    fprintf( stderr, catgets(catd, S_DIRMGT1, DIRMGT6, 
                        "%s: dir_read() read error; [%d] %s; terminating\n"),
                        Prog, errno, ERR_MSG );
                    exit( 1 );
		}
            }
    ...
    )
{
    ...
    if (ino == 0) {
        fprintf( stderr, catgets(catd, S_DIRMGT1, DIRMGT7, 
            "file <%s> not found; will not extract it\n"), name );
        return ERROR;
    }
